...
LOG_FILE="${LOG_DIR}/qwen3.8-27b.log"
...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if kill -0 "${EXISTING_PID}" 2>/dev/null; then
    echo "Qwen server is already running (PID ${EXISTING_PID})."
    exit 0
  fi
  rm -f "${PID_FILE}"
fi

nohup "${PROJECT_DIR}/scripts/serve.sh" >"${LOG_FILE}" 2>&1 &
SERVER_PID=$!
echo "${SERVER_PID}" >"${PID_FILE}"

echo "Started Qwen3.8-27B (PID ${SERVER_PID})."
echo "Log: ${LOG_FILE}"
